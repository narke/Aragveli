...
x86_paging_setup(void)
{
	...
	for (uint32_t pde = 0; pde < 1024; pde++)
	{
		for (uint32_t pte = 0; pte < 1024; pte++)
		{
			...
			page_tables[pde][pte] = physical_addr | PAGE_PRESENT_RW | PAGE_USER_ACCESSIBLE;
		}
		page_directory[pde] = ((uint32_t)&page_tables[pde]) | PAGE_PRESENT_RW | PAGE_USER_ACCESSIBLE;
	}
